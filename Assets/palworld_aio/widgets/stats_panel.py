...
class StatsPanel (QWidget ):
    ...
    def _copy_stats_to_clipboard (self ):
        ...
        try :
            ...
            self ,
            t ('status.copy_success_title')if t else 'Copied',
            t ('status.copy_success_body')if t else 'Stats copied to clipboard!'
            )
        except Exception :
            QMessageBox .warning (
            self ,
            t ('status.copy_fail_title')if t else 'Error',
            t ('status.copy_fail_body')if t else 'Failed to copy to clipboard.'
            )
